...
class Grafo():
    ...
    def barrido_parejas_jedi(self, peso):
        aux = self.__inicio
        ya_visitados = []
        while(aux is not None):
            ara = aux.adyacentes.get_inicio()
            while (ara is not None):
                peso_aux = ara.peso
                pareja = [aux.info, ara.info]
                pareja_invertida = [ara.info, aux.info]
                if (peso_aux >= peso) and (pareja not in ya_visitados) and (pareja_invertida not in ya_visitados):
                    print(aux.info + ' y ' + ara.info + ', que aparecieron juntos en ' + str(peso_aux) + ' episodios')
                    ya_visitados.append(pareja)
                ara = ara.sig
            aux = aux.sig
        return ya_visitados
    ...
    def encontrar_coincidencias(self, almacenador_paises):
        aux = self.__inicio
